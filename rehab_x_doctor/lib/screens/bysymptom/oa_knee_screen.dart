...
class OAKneeScreen extends StatefulWidget {
  const OAKneeScreen({super.key});

  @override
  State<OAKneeScreen> createState() => _OAKneeScreenState();
}

class _OAKneeScreenState extends State<OAKneeScreen> {
  final ExerciseService _exerciseService = ExerciseService();
  List<Exercise> _oaKneeExercises = [];
  ...
  void initState() {
    super.initState();
    _loadOAKneeExercises();
  }

  Future<void> _loadOAKneeExercises() async {
    try {
      final allExercises = await _exerciseService.getExercises();
      // Filter exercises for OA knee symptom
      final oaKneeExercises = allExercises.where((exercise) {
        return exercise.symptom?.toLowerCase().contains('oa_knee') == true ||
               exercise.symptom?.contains('ข้อเข่าเสื่อม') == true;
      }).toList();
      
      setState(() {
        _oaKneeExercises = oaKneeExercises;
        _isLoading = false;
        _applyFilters();
      });
    } catch (e) {
  ...
  void _applyFilters() {
    setState(() {
      _filteredExercises = _oaKneeExercises.where((exercise) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Rehab X - OA Knee Exercises', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.white)),
        actions: [
          IconButton(
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: _loadOAKneeExercises,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'No OA knee exercises available.',
