...
class LowBackPainScreen extends StatefulWidget {
  const LowBackPainScreen({super.key});

  @override
  State<LowBackPainScreen> createState() => _LowBackPainScreenState();
}

class _LowBackPainScreenState extends State<LowBackPainScreen> {
  final ExerciseService _exerciseService = ExerciseService();
  List<Exercise> _lowBackPainExercises = [];
  ...
  void initState() {
    super.initState();
    _loadLowBackPainExercises();
  }

  Future<void> _loadLowBackPainExercises() async {
    try {
      final allExercises = await _exerciseService.getExercises();
      // Filter exercises for low back pain symptom
      final lowBackPainExercises = allExercises.where((exercise) {
        return exercise.symptom?.toLowerCase().contains('low_back_pain') == true ||
               exercise.symptom?.contains('ปวดหลังส่วนล่าง') == true;
      }).toList();
      
      setState(() {
        _lowBackPainExercises = lowBackPainExercises;
        _isLoading = false;
        _applyFilters();
      });
    } catch (e) {
  ...
  void _applyFilters() {
    setState(() {
      _filteredExercises = _lowBackPainExercises.where((exercise) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Rehab X - Low Back Pain Exercises', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.white)),
        actions: [
          IconButton(
  ...
  Widget _buildBody() {
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: _loadLowBackPainExercises,
    ...
    if (_lowBackPainExercises.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              'No low back pain exercises available.',
