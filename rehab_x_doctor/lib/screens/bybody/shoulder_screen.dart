...
// Changed class name from NeckScreen to ShoulderScreen
...
class _ShoulderScreenState extends State<ShoulderScreen> {
  ...
  List<Exercise> _shoulderExercises = [];
  ...
  void initState() {
    super.initState();
    // Changed method call
    _loadShoulderExercises();
  }

  // Changed method name and logic
  Future<void> _loadShoulderExercises() async {
    try {
      final allExercises = await _exerciseService.getExercises();
      // **** IMPORTANT: Updated filtering logic for 'shoulder' and 'หัวไหล่' ****
      final shoulderExercises = allExercises.where((exercise) {
        return exercise.bodyPart?.toLowerCase().contains('shoulder') == true ||
               exercise.bodyPart?.toLowerCase().contains('หัวไหล่') == true ;
      }).toList();
      
      setState(() {
        _shoulderExercises = shoulderExercises;
        _isLoading = false;
        _applyFilters();
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Rehab X - Shoulder Exercises', style: Theme.of(context).textTheme.titleLarge?.copyWith(color: Colors.white)),
        actions: [
          IconButton(
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
              ...
              onPressed: _loadShoulderExercises,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              'No shoulder exercises available.',
