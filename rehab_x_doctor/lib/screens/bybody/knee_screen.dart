...
class _KneeScreenState extends State<KneeScreen> {
  ...
  Future<void> _loadKneeExercises() async {
    try {
      final allExercises = await _exerciseService.getExercises();
      final kneeExercises = allExercises.where((exercise) {
        return exercise.bodyPart?.toLowerCase().contains('knee') == true ||
               exercise.bodyPart?.toLowerCase().contains('เข่า') == true;
      }).toList();

      setState(() {
        _kneeExercises = kneeExercises;
        _isLoading = false;
        _applyFilters();
      });
    } catch (e) {
  ...
  void _applyFilters() {
    setState(() {
      _filteredExercises = _kneeExercises.where((exercise) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Rehab X - Knee Exercises',
  ...
  Widget _buildBody() {
    ...
    if (_kneeExercises.isEmpty) {
      return _buildEmptyMessage('No knee exercises available.');
    }
